...
import type { MediaItem, Post } from "@/lib/content";
...
export function StudioMediaManager({ initialMediaItems, draftPosts }: StudioMediaManagerProps) {
    ...
    <div className="grid gap-6">
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <article className="rounded-[2rem] border border-[var(--color-line)] bg-white p-8 shadow-[0_14px_40px_rgba(10,61,98,0.08)]">
          ...
          <h2 className="mt-4 font-heading text-3xl font-semibold text-[var(--color-ink)]">Manage existing images and public visibility.</h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--color-muted)]">
            Every image can either stay as a private studio asset or become visible on the public Media page. New posts now start from the Posts section.
          </p>

          <div className="mt-6 grid gap-3 text-sm leading-7 text-[var(--color-muted)]">
            <p><span className="font-semibold text-[var(--color-ink)]">Private studio asset:</span> use it for insight/news covers, portraits, or poster design.</p>
            <p><span className="font-semibold text-[var(--color-ink)]">Public post:</span> show it on the Media page so people can like, comment, and share it.</p>
          </div>
          ...
          >
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="grid gap-2 text-sm font-medium text-[var(--color-ink)]">
                Type
                <input name="type" placeholder="Post" className="studio-input" />
              </label>
            </div>
            <label className="grid gap-2 text-sm font-medium text-[var(--color-ink)]">
              Public caption
              <textarea name="caption" rows={3} placeholder="Only needed if this may become a public post." className="studio-input min-h-24" />
            </label>
        ...
        <article className="rounded-[2rem] border border-[var(--color-line)] bg-white p-8 shadow-[0_14px_40px_rgba(10,61,98,0.08)]">
          ...
          <div className="mt-8 grid gap-4 text-sm leading-7 text-[var(--color-muted)]">
            ...
            <p><span className="font-semibold text-[var(--color-ink)]">Public posts:</span> {mediaItems.filter((item) => item.publicPost).length}</p>
            <p><span className="font-semibold text-[var(--color-ink)]">Draft or scheduled editorial items:</span> {draftPosts.length}</p>
          </div>

          <div className="mt-8 rounded-[1.4rem] bg-[var(--color-surface)] p-5 text-sm leading-7 text-[var(--color-muted)]">
            If an image feels like a social/photo moment, make it a <span className="font-semibold text-[var(--color-ink)]">public post</span>. If it is only for design or publishing support, keep it private.
          </div>
        </article>
      </section>
